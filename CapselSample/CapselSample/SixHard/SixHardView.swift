...
struct SixHardView: View {
    @ObservedObject var manager: SixHerddispManager = .DispManager
    ...
    var body: some View {
        if manager.questionview == 1 {
            SixHardQuestionView(viewModel: viewModel, questionNum: 2)
                .frame(height: phone.h*0.85)
        }else if manager.questionview == 3{
            SixHardClear(viewModel: viewModel, rokunenn: rokunenn,dataManeger: dataManeger)
