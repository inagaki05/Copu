...
struct SixNormalView: View {
    @ObservedObject var manager: SixNormaldispManager = .DispManager
    @ObservedObject var rokunenn: rokunenViewMOdel
    //
    @ObservedObject var dataManeger: DataManeger
    // 背景画像の名前
    let aasajl = ["flooring1","flooring2","flooring3","flooring4"]
    // 背景の座標比率リスト
    ...
    var body: some View {
        if manager.questionview == 1 {
            SixNormalQuestionView(viewModel: viewModel, questionNum: 2)
                .frame(height: phone.h*0.85)
        }else if manager.questionview == 3{
            SixClear(viewModel: viewModel,rokunenn: rokunenn, dataManeger: dataManeger)
        }else{
